...
GitHub 表单的课程栏为必填文本字段, 网站链接通过 course 参数带入课程名称与代码, 与页面地址一同填写, 不再使用下拉框预选。直接打开表单时预填“按页面地址自动匹配”; 已填对应页面地址可保留该文字, 没有页面地址时可填写课程名称与代码供人工定位。网站仍须从课程与页面列表选定才能继续, 搜索框内任意文字不能直接提交。

python scripts/contributions/catalog.py --write 同步两份 Issue 表单的课程字段定义, 构建会校验一致性与课程标签唯一性。课程映射和页面增删在构建时更新 contribute/catalog.json, 不再把课程选项复制到 GitHub 表单。课程代码用于同名消歧, 未发布的草稿和退休别名不加入页面列表。
