...
GitHub 原生表单中的课程为必选固定下拉框, 页面地址通过文本字段带入。原生下拉字段的 URL 预选不能作为可靠前提, 网站提示投稿者在 GitHub 确认课程; 动态联动与多页选择由站内完成。无脚本或目录加载失败时可直接用原生表单。课程不在列表时选人工核对项, 新资料尚无页面时可选课程主页。
...
处理脚本在任何模型调用或产物写入前, 核对表单的课程名称与代码、本站页面 URL 和维护者选择的目标文件。任一不匹配都停止并留给人工处理; 公共页面和未知课程可投稿, 本版自动候选仍限已登记的现有课程页。Issue 提交后正文可编辑, 下拉框不能代替处理端验证。
