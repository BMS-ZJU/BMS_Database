...
当前交付状态: 投稿入口、人工审批、持久账本、三家官方适配与审阅产物代码已具备, **真实调用默认关闭, 累计额度为零**。独立测试中的 DeepSeek 本地真实调用通过过一次; 正式版完整 GitHub 审批、账本和模型链路尚未云端验收。OpenAI 与 Gemini 只有模拟响应验证。代码可先入库, 收件和人工修改无需模型配置。

## 课程与页面选择

站内下拉框支持输入关键词筛选，按课程名称、代码、页面标题或学年匹配。搜索文字必须经点击或回车选定为现有选项才可继续，未匹配文字不会进入目标URL；修改课程搜索会清空旧页面。保留键盘、中文输入法和触摸操作。此筛选仅在站内实现，不改变GitHub原生固定课程下拉框。
...
GitHub 原生表单保留课程下拉框, 默认“按页面地址自动匹配”, 页面地址通过文本字段带入。从网站进入时无需再选课程, 不依赖 GitHub 是否接受下拉字段的 URL 预选; 动态联动与多页选择由站内完成。无脚本或目录加载失败时可直接用原生表单。课程不在列表时选人工核对项, 新资料尚无页面时可选课程主页。

课程选项由 python scripts/contributions/catalog.py --write 同步到两份 Issue 表单。修改课程映射后运行该命令, 并将表单差异一并提交; 构建会校验一致性。普通页面的增删和改名在构建时自动更新 contribute/catalog.json, 不修改源文件。课程代码用于同名消歧, 未发布的草稿和退休别名不加入页面列表。

处理脚本在任何模型调用或产物写入前, 核对本站页面 URL 和维护者选择的目标文件。默认匹配只接受与目标完全对应的本站页面地址, 不从标题推测课程, 不因维护者填写了目标就补全缺失地址; 手动选择课程时另核对课程名称与代码。原始选择和页面地址保留在快照中, 修改任一项需重新批准。地址缺失或任一不匹配均停止并留给人工处理; 公共页面和未知课程可投稿, 本版自动候选仍限已登记的现有课程页。Issue 提交后正文可编辑, 下拉框不能代替处理端验证。
...
2. 在 Actions 的“整理公开投稿”中选择 operation = snapshot, 填写 issue_number 和 target_page。目标仍是一页已登记的课程 Markdown, 如 docs/mandatory/neuroscience/index.md。此操作不提供模型密钥, 不调用模型。
...
演示复用测试夹具和实际快照、批准、HMAC账本、输出校验及内存构建函数, 在进程中禁止发起 socket 连接, 不读取真实API密钥。GitHub运行/身份、内存Git图和固定模型响应都是模拟; 不能据此声称云端身份、持久存储或真实API通过。演示模块不被生产工作流调用, 没有给付费入口增加模拟/跳过校验开关。私有测试仓库的 mock/real 工作流、模拟账本和固定响应属于测试部署, 本次不迁入正式工作流; 正式版的无需密钥演示仍使用这里的本机入口。结束演示服务按 Ctrl+C, 已生成的审阅文件保留在指定输出目录。
...
本地还应运行严格构建并检查已有投稿页面交互。首次云端启用前先保持付费开关关闭, 验证 snapshot、维护者权限、账本初始化和产物下载; 再按明确授权做合成公开样例的最小真实调用。本轮不自动发起此调用。账户角色、分支保护、Actions 权限、实际 API 用量结构、延迟和账单均需在真实环境中确认。
