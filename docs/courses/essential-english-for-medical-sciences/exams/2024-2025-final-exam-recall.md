---
title: 医学科学英语基础 2024-2025 学年期末回忆卷
---

# 2024-2025 学年医学科学英语基础期末回忆卷

<!--
source-attribution
贡献者：朱晓彤（回忆卷作者、资料上传）。
原始出处：《24-25医科英不完全回忆卷.pdf》。
资料性质：以教材题目截图整理的回忆卷。
整理说明：保留原材料中的题干、选项、匹配分组、词库与示例答案；各题型内按阅读顺序连续编号，保留连线和填空的原有分组。
来源记录：RESOURCE-AUDIT-R09-20260909 / R09a。
-->

!!! info "回忆卷说明"

    本页依据回忆整理。各题型内按阅读顺序连续编号，题号不代表原卷题号。
    原材料标注选择题 32 题、连线题 100 条；填空共 18 分，每题 0.4 分（45 题）。材料实际收录了 34 道选择题、14 组共 70 条连线和 24 条填空，以下全部保留。

回忆卷编写、上传：**朱晓彤**。

---

## 一、选择题 {: .exam-section .exam-section--choice }

*共 32 题，每题 1 分。*

1. A word part at the end of a word is the

    **A.** prefix  
    **B.** adjective  
    **C.** insertion  
    **D.** suffix

2. The word below that has a hard *g* is

    **A.** grip  
    **B.** page  
    **C.** gem  
    **D.** judge

3. The suffixes *-ic*, *-ous*, *-al*, and *-oid* are found in

    **A.** adjectives  
    **B.** nouns  
    **C.** verbs  
    **D.** roots

4. The prefix in the word *microscopic* is

    **A.** mic-  
    **B.** scop-  
    **C.** micro-  
    **D.** pic-

5. *Epi-* in the term *epigastric* is a

    **A.** word root  
    **B.** prefix  
    **C.** suffix  
    **D.** combining form

6. The *-oid* in the term *xiphoid* is a

    **A.** root  
    **B.** prefix  
    **C.** derivation  
    **D.** suffix

7. The adjective for *larynx* is

    **A.** larynxic  
    **B.** laryngeal  
    **C.** larynal  
    **D.** largeal

8. The combining form for *thorax* (chest) is

    **A.** thorax/o  
    **B.** thor/o  
    **C.** thorac/o  
    **D.** thori/o

9. The root that means “cell” is

    **A.** spher  
    **B.** aden  
    **C.** cyt  
    **D.** gen

10. The process of body cell division is called

    **A.** separation  
    **B.** segregation  
    **C.** mitosis  
    **D.** gestation

11. The substance that makes up the cell’s genetic material is

    **A.** DNA  
    **B.** mineral  
    **C.** base  
    **D.** neurons

12. The root *brachi/o* refers to the

    **A.** head  
    **B.** spinal cord  
    **C.** leg  
    **D.** arm

13. A disease that has a sudden and severe onset is described as

    **A.** chronic  
    **B.** mild  
    **C.** acute  
    **D.** infectious

14. Abnormal and uncontrolled growth of tissue is termed

    **A.** anemia  
    **B.** neoplasia  
    **C.** parasitism  
    **D.** toxicity

15. The sum of all body defenses against infectious disease is termed

    **A.** pyosis  
    **B.** complementation  
    **C.** secretion  
    **D.** immunity

16. A reason for not using a specific drug is a

    **A.** prescription  
    **B.** counter-purpose  
    **C.** contraindication  
    **D.** prognosis

17. The endocardium is the tissue lining the heart’s chambers. Endocarditis refers to a(n) ______ of this lining.

    **A.** narrowing  
    **B.** inflammation  
    **C.** overgrowth  
    **D.** thinning

18. The term for a condition or disease of unknown etiology is

    **A.** iatrogenic  
    **B.** chronic  
    **C.** acute  
    **D.** idiopathic

19. The basic cell of the nervous system is a(n)

    **A.** myofiber  
    **B.** neuron  
    **C.** osteoblast  
    **D.** chondrocyte

20. Involuntary responses are controlled by the

    **A.** somatic nervous system  
    **B.** voluntary nervous system  
    **C.** autonomic nervous system  
    **D.** diaphragm

21. A disorder, often of unknown cause, characterized by seizures is called

    **A.** cystic fibrosis  
    **B.** spina bifida  
    **C.** epilepsy  
    **D.** thyrotoxicosis

22. An instrument used to study the electric activity of the brain is the

    **A.** electrocardiograph  
    **B.** electroencephalograph  
    **C.** CT scanner  
    **D.** sonograph

23. The secretions of the endocrine glands are called

    **A.** enzymes  
    **B.** sera  
    **C.** lymph  
    **D.** hormones

24. The small gland in the brain that controls other glands is the

    **A.** thymus  
    **B.** pituitary  
    **C.** appendix  
    **D.** corpus luteum

25. Gigantism results from overproduction of

    **A.** erythropoietin  
    **B.** oxytocin  
    **C.** growth hormone  
    **D.** prolactin

26. Diabetes mellitus involves the hormone insulin, which is made in the

    **A.** kidney  
    **B.** seminal vesicle  
    **C.** thymus  
    **D.** pancreas

27. Parathyroid hormone regulates

    **A.** magnesium  
    **B.** calcium  
    **C.** cortisol  
    **D.** glucose

28. Cushing disease is overactivity of the

    **A.** thyroid  
    **B.** parathyroid  
    **C.** adrenal cortex  
    **D.** adrenal medulla

29. The cardiovascular system includes the heart and

    **A.** lungs  
    **B.** digestive organs  
    **C.** blood vessels  
    **D.** endocrine system

30. A vessel that carries blood away from the heart is a(n)

    **A.** vein  
    **B.** chamber  
    **C.** lymph node  
    **D.** artery

31. The medical term for a “heart attack” is

    **A.** cerebrovascular accident  
    **B.** myocardial infarction  
    **C.** aneurysm  
    **D.** pneumonia

32. The accumulation of fatty deposits in the lining of a vessel is called

    **A.** obesity  
    **B.** stent  
    **C.** atherosclerosis  
    **D.** angiogenesis

33. Erythrocyte is the scientific name for a

    **A.** white blood cell  
    **B.** lymphocyte  
    **C.** red blood cell  
    **D.** muscle cell

34. Substances produced by immune cells that counteract microorganisms and other foreign materials are called

    **A.** antigens  
    **B.** antibodies  
    **C.** anticoagulants  
    **D.** Rh factors

## 二、连线题（50 分） {: .exam-section .exam-section--short data-toc-label="二、连线题" }

*共 100 题，每题 0.5 分。两栏内容需自行匹配。*

<div class="matching-group">
    <p class="matching-title"><strong>第 1 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="1" aria-label="词语">
            <li>erythematous</li>
            <li>melanoma</li>
            <li>xanthocyte</li>
            <li>cyanotic</li>
            <li>leukocyte</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>cell with yellow color</li>
            <li>having a bluish discoloration</li>
            <li>darkly pigmented tumor</li>
            <li>red in color</li>
            <li>white blood cell</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 2 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="6" aria-label="词语">
            <li>ATP</li>
            <li>DNA</li>
            <li>nucleoplasm</li>
            <li>nucleus</li>
            <li>cytoplasm</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>control region of the cell</li>
            <li>material that holds the cellular organelles</li>
            <li>energy compound of the cells</li>
            <li>genetic material</li>
            <li>material that fills the nucleus</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 3 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="11" aria-label="词语">
            <li>blastocyte</li>
            <li>ribosomes</li>
            <li>mitochondria</li>
            <li>mitosis</li>
            <li>lysosome</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>immature cell</li>
            <li>organelles that produce ATP</li>
            <li>organelles that contain RNA</li>
            <li>small cellular body containing digestive enzymes</li>
            <li>cell division</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 4 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="16" aria-label="词语">
            <li>autotroph</li>
            <li>papilliform</li>
            <li>amorphous</li>
            <li>atrophy</li>
            <li>somatic</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>resembling a nipple</li>
            <li>having no specific form</li>
            <li>wasting of tissue</li>
            <li>pertaining to the body</li>
            <li>organism that can manufacture its own food</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 5 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="21" aria-label="词语">
            <li>cardiomegaly</li>
            <li>neuroma</li>
            <li>carcinophobia</li>
            <li>encephalitis</li>
            <li>hemorrhagic</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>pertaining to profuse flow of blood</li>
            <li>fear of cancer</li>
            <li>tumor of a nerve</li>
            <li>enlargement of the heart</li>
            <li>inflammation of the brain</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 6 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="26" aria-label="词语">
            <li>blepharoptosis</li>
            <li>hemostasis</li>
            <li>toxoid</li>
            <li>lesion</li>
            <li>ectasia</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>local wound or injury</li>
            <li>stoppage of blood flow</li>
            <li>dropping of the eyelid</li>
            <li>like a poison</li>
            <li>dilatation</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 7 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="31" aria-label="词语">
            <li>hyperpyrexia</li>
            <li>potentiation</li>
            <li>chronotropic</li>
            <li>anaphylaxis</li>
            <li>efficacy</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>extreme allergic reaction</li>
            <li>effectiveness</li>
            <li>affecting timing</li>
            <li>combined drug action to greater effect</li>
            <li>abnormally high body temperature</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 8 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="36" aria-label="词语">
            <li>nosocomial</li>
            <li>iatrogenic</li>
            <li>fistula</li>
            <li>polyp</li>
            <li>effusion</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>abnormal passageway</li>
            <li>escape of fluid into a cavity</li>
            <li>tumor attached by a thin stalk</li>
            <li>acquired in a hospital</li>
            <li>caused by effects of treatments</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 9 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="41" aria-label="词语">
            <li>amnesia</li>
            <li>euphoria</li>
            <li>claustrophobia</li>
            <li>ataxia</li>
            <li>lethargy</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>fear of being enclosed</li>
            <li>state of sluggishness</li>
            <li>loss of memory</li>
            <li>lack of muscle coordination</li>
            <li>sense of elation</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 10 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="46" aria-label="词语">
            <li>parathyroid</li>
            <li>posterior pituitary</li>
            <li>hypothalamus</li>
            <li>anterior pituitary</li>
            <li>pineal</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>gland that is regulated by light</li>
            <li>small gland that acts to increase blood calcium levels</li>
            <li>part of the brain that controls the pituitary</li>
            <li>gland that secretes ACTH</li>
            <li>gland that releases oxytocin</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 11 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="51" aria-label="词语">
            <li>ketoacidosis</li>
            <li>adenoma</li>
            <li>Cushing syndrome</li>
            <li>acromegaly</li>
            <li>diabetes insipidus</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>disorder that results from excess growth hormone</li>
            <li>disorder caused by insufficient release of ADH</li>
            <li>a result of uncontrolled diabetes</li>
            <li>disorder caused by overactivity of the adrenal cortex</li>
            <li>neoplasm of a gland</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 12 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="56" aria-label="词语">
            <li>lumen</li>
            <li>pericardium</li>
            <li>apex</li>
            <li>vena cava</li>
            <li>valve</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>vessel that empties into the right atrium</li>
            <li>fibrous sac around the heart</li>
            <li>structure that keeps fluid moving forward</li>
            <li>central opening of a vessel</li>
            <li>lower, pointed region of the heart</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 13 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="61" aria-label="词语">
            <li>hypokalemia</li>
            <li>natriuresis</li>
            <li>ferric</li>
            <li>siderosis</li>
            <li>azoturia</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>condition involving iron deposits</li>
            <li>deficiency of potassium in the blood</li>
            <li>urinary excretion of sodium</li>
            <li>urinary excretion of nitrogenous compounds</li>
            <li>pertaining to iron</li>
            </ol>
        </div>
    </div>
</div>

<div class="matching-group">
    <p class="matching-title"><strong>第 14 组</strong></p>
    <div class="matching-columns">
        <div>
            <p class="matching-label">词语</p>
            <ol class="matching-terms" start="66" aria-label="词语">
            <li>hemophilia</li>
            <li>hemostasis</li>
            <li>hypersensitivity</li>
            <li>thalassemia</li>
            <li>purpura</li>
            </ol>
        </div>
        <div>
            <p class="matching-label">待选释义</p>
            <ol class="matching-definitions" type="A" aria-label="待选释义">
            <li>allergy</li>
            <li>hereditary form of anemia</li>
            <li>stoppage of blood flow</li>
            <li>hereditary clotting disorder</li>
            <li>bleeding into the tissues</li>
            </ol>
        </div>
    </div>
</div>

## 三、填空题（18 分） {: .exam-section .exam-section--short data-toc-label="三、填空题" }

*共 45 题；每题 0.4 分。*

**第一组**

Write words for the following definitions using the word parts provided. A combining vowel is included. Each word part can be used more than once.

> -itis · -logy · -ptosis · nephr · -o · gastr · cardi · neur-

<ol start="1" class="exam-question-list exam-question-list--short" markdown="1">
<li markdown="1">Inflammation of the stomach: gastritis（原材料示例）</li>
<li markdown="1">Study of the nervous system: ______</li>
<li markdown="1">Dropping of the kidney: ______</li>
<li markdown="1">Study of the kidney: ______</li>
<li markdown="1">Inflammation of a nerve: ______</li>
<li markdown="1">Downward displacement of the heart: ______</li>
</ol>

**第二组**

> -oid · amyl/o · muc/o · aden/o · -ase · lip/o · leuk/o · histi/o · blast

<ol start="7" class="exam-question-list exam-question-list--short" markdown="1">
<li markdown="1">Like or resembling a gland: adenoid（原材料示例）</li>
<li markdown="1">Immature white blood cell: ______</li>
<li markdown="1">Enzyme that digests fat: ______</li>
<li markdown="1">Resembling mucus: ______</li>
<li markdown="1">Cell that gives rise to tissue: ______</li>
<li markdown="1">Enzyme that digests starch: ______</li>
<li markdown="1">Resembling starch: ______</li>
</ol>

**第三组**

> -plegia · myel/o · -a- · -itis · dys- · brady- · my/o · tetra- · -paresis · -phasia · gangli/o · hemi-

<ol start="14" class="exam-question-list exam-question-list--short" markdown="1">
<li markdown="1">paralysis of the spinal cord: ______</li>
<li markdown="1">lack of speech: ______</li>
<li markdown="1">partial paralysis of one side of the body: ______</li>
<li markdown="1">muscle weakness: ______</li>
<li markdown="1">abnormal or difficult speech production: ______</li>
<li markdown="1">paralysis of a ganglion: ______</li>
<li markdown="1">paralysis of all four limbs: ______</li>
<li markdown="1">inflammation of the spinal cord: ______</li>
<li markdown="1">slowness of speech: ______</li>
<li markdown="1">paralysis of one side of the body: ______</li>
<li markdown="1">inflammation of a ganglion: ______</li>
</ol>
