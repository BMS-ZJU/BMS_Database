(() => {
  ...
  const validateSource = (value, depth = 0) => {
    if (depth > 8) throw new Error(sourceError);
    let url;
    let path;
    try {
    ...
    const match = /^(courses|mandatory|elective)\/([^/]+)\/(exams|quizzes)\/([^/]+?)(?:\/|\.html)$/.exec(path);
    ...
    if (Object.hasOwn(pathMigrations, path)) {
      const destination = new URL(pathMigrations[path], siteRoot);
      destination.hash = url.hash || destination.hash;
      return validateSource(destination.href, depth + 1);
    }
    if (match[1] !== "courses") throw new Error(sourceError);
    return url;
  };
