...
(() => {
  ...
  function resolvePageReference(data, value) {
    let reference = value || "";
    if (reference.startsWith("docs/")) reference = reference.slice(5);
    let [path, ...fragmentParts] = reference.split("#");
    let fragment = fragmentParts.join("#");
    const seen = new Set();
    while (Object.hasOwn(data.aliases || {}, path)) {
      if (seen.has(path)) return {page: null, fragment: ""};
      seen.add(path);
      const [next, ...parts] = data.aliases[path].split("#");
      path = next;
      fragment ||= parts.join("#");
    }
    return {page: data.pages.find(page => page.path === path), fragment};
  }
  async function mount() {
    ...
    try {
      ...
      const requested = resolvePageReference(data, params.get("page"));
      const selected = requested.page;
      ...
      function addCard(initial) {
        ...
        function update() {
          ...
          const itemUrl = item ? new URL(item.url) : null;
          if (itemUrl && item.path === initial?.page?.path && initial.fragment) {
            itemUrl.hash = initial.fragment;
          }
          ...
          if (valid) {
            ...
            url.searchParams.set("page", itemUrl ? itemUrl.href : "");
          ...
          if (itemUrl) view.href = itemUrl.href;
        }
      ...
      addCard({kind: params.get("kind"), page: selected, fragment: requested.fragment});
      loading.hidden = true;
      add.hidden = false;
